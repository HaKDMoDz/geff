...
public class Cubeat : MonoBehaviour
{
    ...
    void uiBackground_onTouchUpInside(UIButton obj)
    {
        Debug.Log("BTouched");
        InputController.Instance.UpdateTouch();
    }
    ...
    void btnMargeArrow_onTouchUpInside(UIButton obj)
    {
        ...
        if (margeCollapsed)
        {
            btnMargeArrow.positionTo(0.5f, new Vector3(0.01f * Screen.width, 0.01f * Screen.height, btnMargeArrow.localPosition.z), Easing.Sinusoidal.easeOut);
            spriteMarge.scaleTo(0.5f, new Vector3(1f, 1f, spriteMarge.localPosition.z), Easing.Sinusoidal.easeOut);

            foreach (UIHorizontalLayout hLayout in listHLayout)
            {
                hLayout.positionTo(0.5f, new Vector3(-Screen.width / 2, hLayout.localPosition.y, hLayout.localPosition.z), Easing.Sinusoidal.easeOut);
            }
        }
        else
        {
            btnMargeArrow.positionTo(0.5f, new Vector3(0.5f * Screen.width, 0.01f * Screen.height, btnMargeArrow.localPosition.z), Easing.Sinusoidal.easeIn);
            spriteMarge.scaleTo(0.5f, new Vector3(50f, 1f, spriteMarge.localPosition.z), Easing.Sinusoidal.easeIn);

            foreach (UIHorizontalLayout hLayout in listHLayout)
            {
                hLayout.positionTo(0.5f, new Vector3(0.01f * Screen.width, hLayout.localPosition.y, hLayout.localPosition.z), Easing.Sinusoidal.easeIn);
            }
        }
    }
